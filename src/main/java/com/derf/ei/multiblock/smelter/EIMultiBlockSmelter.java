...
/**
 * Smelters are somewhat different from the furnace. 
 * It is used to do ore multiplications. 
 * 2x low tier 
 * 3x mid tier 
 * 4x high tier
 * @author Fred
 *
 */
public class EIMultiBlockSmelter extends EIMultiBlock {
	...
	private int getOutSlotLocation(IInventory inv, Item inp) {
		int i = 0;
		...
		for(; i < inv.getSizeInventory(); i++) {
			...
			if(inv.getStackInSlot(i).getItem() == inp &&
			   inv.getStackInSlot(i).getMaxStackSize() > inv.getStackInSlot(i).stackSize) {
				break;
			}
	...
	private boolean canCook(ItemStack item) {
		...
		Item temp = EICraftingMultiBlock.getSmelterResult(item);
	...
	private void updateInventory(World world) {
		...
		if(inputInv != null && outputInv != null) {
			...
			if(!this.isOutputInventoryFull(outputInv, inputStack) && inputStack != null) {
				Item outputStack = EICraftingMultiBlock.getSmelterResult(inputStack);
				int i = this.getOutSlotLocation(outputInv, outputStack);
				
				
				if(outputInv.getStackInSlot(i) == null) {
					outputInv.setInventorySlotContents(i, new ItemStack(outputStack, this.quantity));
				} else if(outputInv.getStackInSlot(i).getItem() == outputStack) {
					outputInv.getStackInSlot(i).stackSize += this.quantity;
				}
